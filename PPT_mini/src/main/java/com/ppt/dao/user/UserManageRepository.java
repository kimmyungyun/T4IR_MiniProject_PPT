...
public class UserManageRepository implements IUserManageRepository {
	...
	public boolean signupUser(User usr) {
		...
		String sql = "INSERT INTO users "
				+ "(u_id, u_password, u_name, u_email, u_birth, u_phone, u_address) "
				+ "VALUES (?, ?, ?, ?, sysdate, ?, ?)";
		jdbcTemplate.update(sql, usr.getId(), usr.getPw(), usr.getName(), usr.getEmail(), usr.getPhoneNum(), usr.getAddress());
		return true;
	}
